...
function makeDate(day: Date, minutes: number) {
...
async function schedule(date: Date, title: string, body: string, kind: string) {
  if (Platform.OS === 'web') return;
...
export async function rescheduleCampusLifeNotifications(config: Schedule) {
  ...
  for (let offset = 0; offset < 14; offset += 1) {
    ...
    for (
      let at = start + config.photoIntervalMinutes;
      at < warning;
      at += config.photoIntervalMinutes
    ) {
      await schedule(
        makeDate(day, at),
